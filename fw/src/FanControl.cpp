...
void FanControl::update_outputs() {
	...
	// Fan 0: OC4C
	if( switch_or & 0b00000001 ) { OCR4C = MAX_TIMER_COUNT; }
	else                         { OCR4C = fan_count[0];    }
	// Fan 1: OC5A
	if( switch_or & 0b00000010 ) { OCR5A = MAX_TIMER_COUNT; }
	else                         { OCR5A = fan_count[1];    }
	// Fan 2: OC5B
	if( switch_or & 0b00000100 ) { OCR5B = MAX_TIMER_COUNT; }
	else                         { OCR5B = fan_count[2];    }
	// Fan 3: OC5C
	if( switch_or & 0b00001000 ) { OCR5C = MAX_TIMER_COUNT; }
	else                         { OCR5C = fan_count[3];    }
	// Fan 4: OC4A
	if( switch_or & 0b00010000 ) { OCR4A = MAX_TIMER_COUNT; }
	else                         { OCR4A = fan_count[4];    }
	// Fan 5: OC3C
	if( switch_or & 0b00100000 ) { OCR3C = MAX_TIMER_COUNT; }
	else                         { OCR3C = fan_count[5];    }
	// Fan 6: OC3B
	if( switch_or & 0b01000000 ) { OCR3B = MAX_TIMER_COUNT; }
	else                         { OCR3B = fan_count[6];    }
	// Fan 7: OC3A
	if( switch_or & 0b10000000 ) { OCR3A = MAX_TIMER_COUNT; }
	else                         { OCR3A = fan_count[7];    }
	// Test Fan: OC4B
	OCR4B = test_count;
...
void FanControl::set_duty_cycle( uint8_t idx, uint8_t dc ) {
	...
	fan_count[idx] = ( MAX_TIMER_COUNT * (uint16_t)dc ) / MAX_DUTY_CYCLE;
...
void FanControl::set_test_duty_cycle( uint8_t dc ) {
	...
	// Calculate the corresponding fan counts
	test_count = ( MAX_TIMER_COUNT * (uint16_t)dc ) / MAX_DUTY_CYCLE;
